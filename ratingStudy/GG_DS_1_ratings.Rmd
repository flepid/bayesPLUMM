...
#raw data
data <- read.csv('...\\data_GG_DS_205.csv')
...
WM_stim_key <- read.csv('...\\GG_DS_1\\WM_stim.csv')
...
#load stim info
DS_df <- read.csv('...\\DS_rhythmInfo_30.csv')
SI_df <- read.csv('...\\SI_rhythmInfo_30.csv')
...
#write.csv(stim_df, '...\\rhythmInfo_60.csv',row.names = FALSE)

```



find participants with low variability in their ratings
```{r}
...
#write.csv(df1, '...\\df_60.csv',row.names = FALSE)
...
#tiff(file="...\\DS_SI_meas.tiff", units = 'in', res = 300,width=8, height =4)
...
#tiff(file="...\\DS_age.tiff", units = 'in', res = 300,width=5, height =4)
